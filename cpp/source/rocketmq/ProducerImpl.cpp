...
RecallReceipt ProducerImpl::recall(const std::string& topic, std::string& recall_handle, std::error_code& ec) noexcept {
  ...
  if (message_queue_list.empty() ||
      !topic_publish_info->selectMessageQueues(message_group, message_queue_list)) {
    SPDLOG_WARN("Failed to select an addressable message queue for topic[{}]", topic);
    ec = ErrorCode::NotFound;
    return RecallReceipt{};
  }
  ...
  auto request = new rmq::RecallMessageRequest();
  request->mutable_topic()->set_resource_namespace(resourceNamespace());
  request->mutable_topic()->set_name(topic);
  ...
  auto callback =
      [&, mtx, cv, topic](const std::error_code& ec, const RecallMessageResponse& response) {
